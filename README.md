...
- **告警中心** — 四种告警规则（慢查询耗时 / 无索引比例 / 查询总量 / 自定义 SQL），ClickHouse 聚合查询触发，Webhook 通知兼容 Alertmanager v4
- **执行计划采集** — 采集周期自动对 top-5 慢查询执行 EXPLAIN，写入 ClickHouse 做历史版本对比，支持 plan diff（访问类型 / 索引 / filesort 标注优化或退化）
...
| `GET  /api/qan/top-queries/?service=xxx` | 慢查询聚合 |
| `GET  /api/qan/plans/?fingerprint=xxx` | 执行计划历史 |
| `POST /api/qan/explain/` | 手动 EXPLAIN |
| `GET  /api/locks/topology/?instance_id=<id>` | 实时锁链拓扑 |
| `GET  /api/locks/history/?instance_id=<id>&hours=1` | 历史锁等待 |
| `GET  /api/alerts/rules/` | 告警规则管理 |
